          ...
          return (
            <div
              ...
              className={`aspect-square rounded-lg flex flex-col items-center justify-center p-1 relative
                ${future ? 'bg-gray-50 cursor-not-allowed' : colorClass + ' cursor-pointer hover:ring-2 hover:ring-blue-400'}
                ${today ? 'ring-2 ring-orange-500' : ''}
              `}
              title={`${date.toLocaleDateString()}: ${count} drinks`}
            >
              <span className={`text-xs font-semibold ${count > 6 ? 'text-white' : 'text-gray-700'}`}>
                {date.getDate()}
              </span>
              {count > 0 && (
                <span className={`text-[10px] ${count > 6 ? 'text-white' : 'text-gray-600'}`}>
                  {count}
                </span>
              )}
            </div>
